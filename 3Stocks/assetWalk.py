...
def driver():
    ...
    tInt = times['day']
    avgVals, volFracsMax, tradeFracsMax, n = initialize('BTCDay.csv',tInt)
    ...
    plt.plot(xVals,openPrices[:num],'g-')
    plt.xlabel('t [day]')
    plt.ylabel('BTC ($)')
...
# initaalize stock as S0
    walk = np.zeros(n)
    price0 = averagePrices[0]
    walk[0] = price0
    walk[1] = averagePrices[1]
    walk[2] = averagePrices[2]
    # correction = 0
...
#   take step
    for ii in range(2,n-1):
        ...
        data1 = averagePrices[ii-1]; data2 = averagePrices[ii];
        mu1,mu2 = deltaMu(averagePrices[:ii])
        ...
        walk[ii+1] = walk[ii] + (mu2-mu1)*random.uniform(0,sigma) + dVar/math.sqrt(abs(dVar))*random.uniform(0,sigma)
